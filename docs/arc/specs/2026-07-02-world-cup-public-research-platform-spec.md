...
- The deterministic public-snapshot Playwright journeys also run Axe's default
  accessibility rules against loaded and unavailable-data states. They report
  zero violations after correcting the document language, muted-text contrast,
  duplicate landmark names, decorative probability-track semantics, and the
  zoom-restricting viewport declaration. Manual screen-reader verification
  remains a separate launch check.
